(ns duckcreek_policy.main
  "Kotodama WASM entrypoint for the Duckcreek_policy clean-room actor (L4) — Clojure port.
  ...
  facts (`duckcreek_policy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "duckcreek_policy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"duckcreek_policy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "duckcreek_policy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :duckcreek_policy-compat/ready)
